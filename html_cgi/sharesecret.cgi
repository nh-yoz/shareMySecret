...
try:
    ...
    arguments = cgi.parse() # Should return response as { 'method': post/get, 'token': string if method=GET }
    if method != 'OPTIONS':
    ...
    if method == 'OPTIONS':
        ...
        if validate_arguments(arguments, { 'allowed': ('method',), 'required': () }):
            if cont_type != 'application/json' or cont_len == 0:
                ...
                if not msg_sent and validate(data):
                    store_secret(data)
    elif method == 'GET':
        if validate_arguments(arguments, { 'allowed': ('method', 'token'), 'required': ('token',) }):
